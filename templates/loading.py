...
<html lang="ru">
    ...
    <body class="d-flex flex-column" style="height: 100vh;">
        ...
        <div class="container-md my-auto">
            <div class="my-auto border rounded p-3" style="height: 100%;">
                <form action="/generate" method="POST" enctype="multipart/form-data" id="form">
                  <h2 class="text-center display-6">Генерация Image2Video</h2>
                  <div class="mb-3">
                      <label for="fileInput" class="form-label">Изображение</label>
    ...
    document.getElementById('send').addEventListener('click', function(event) {
        ...
        formData.append('userId', 'panel');
        formData.append('appId', 'panel');
        ...
        formData.append('cfg_scale', parseFloat(document.getElementById('cfg_scale').value));
        formData.append('aspect_ratio', document.getElementById('aspect_ratio').value);
        formData.append('duration', document.getElementById('duration').value);
        formData.append('mode', document.getElementById('mode').value);

        // Append camera control settings
        const cameraType = document.getElementById('camera_type').value === "" ? null : document.getElementById('camera_type').value;
        formData.append('camera_control[type]', cameraType); // Use a bracket notation for nested fields
        formData.append('camera_control[config][horizontal]', parseFloat(document.getElementById('horizontal').value) || 0);
        formData.append('camera_control[config][vertical]', parseFloat(document.getElementById('vertical').value) || 0);
        formData.append('camera_control[config][pan]', parseFloat(document.getElementById('pan').value) || 0);
        formData.append('camera_control[config][tilt]', parseFloat(document.getElementById('tilt').value) || 0);
        formData.append('camera_control[config][roll]', parseFloat(document.getElementById('roll').value) || 0);
        formData.append('camera_control[config][zoom]', parseFloat(document.getElementById('zoom').value) || 0);

        // Append the file from the file input
        const fileInput = document.getElementById('fileInput'); // Replace with your file input ID
